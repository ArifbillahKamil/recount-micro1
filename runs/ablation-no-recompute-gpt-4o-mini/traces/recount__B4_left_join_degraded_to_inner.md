...
```sql
SELECT COUNT(*) AS orders_seen, COALESCE(SUM(r.amount_cents), 0) AS refunded_cents FROM orders o LEFT JOIN refunds r ON r.order_id = o.order_id
```
...
```json
{
  ...
  "corrected": "orders_seen | refunded_cents\n------------+---------------\n1500        | 308416250     "
}
```
